...
import type { RunnerAdmissionPolicy, RunnerExecutionIsolation, RunnerWorktreePorts } from "./config.js";
...
export class SessionManager {
   ...
   * provider-auth block. A restarted runner reconstructs these from control-plane redelivery. */
  private readonly providerAuthDurables = new Map<string, DurableCommandLifecycle>();
  ...
  constructor(
    ...
    this.worktreePortAllocator = new WorktreePortAllocator(this.stateDir, this.worktreePorts);
  ...
  worktreePortRuntime(): RunnerWorktreePorts & { capacity: number } {
    return this.worktreePortAllocator.runtime();
  }

  setWorktreeShellRetirement(
    retire: (sessionId: string, context: AgentContext, path: string) => Promise<void>,
  ): void {
    this.retireWorktreeShells = retire;
  }

  private worktreePortOwner(sessionId: string, worktreeId: string): string {
    return `${sessionId}\0${worktreeId}`;
  }

  private ensureWorktreePortBlock(meta: SessionMeta, worktree: SessionWorktreeView): WorktreePortBlock | undefined {
    if (worktree.source === "attached") return undefined;
    const owner = this.worktreePortOwner(meta.sessionId, worktree.id);
    const block = this.worktreePortAllocator.allocate(owner, worktree.portBlock);
    worktree.portBlock = block;
    return block;
  }
  ...
  private persistWorktreeHookSnapshot(
    ...
    snapshot: WorktreeHookSnapshot,
  ): void {
    const latest = this.store.readMeta(sessionId);
    if (!latest) return;
    this.store.patchMeta(sessionId, {
      worktreeHooks: { ...(latest.worktreeHooks ?? {}), [worktree.id]: structuredClone(snapshot) },
    });
  }

  private worktreeHookSnapshot(meta: SessionMeta, worktree: SessionWorktreeView): WorktreeHookSnapshot | undefined {
    return meta.worktreeHooks?.[worktree.id];
  }

  /** Upgrade a #637-era identity from its immutable base commit. The live worktree config is never
   * consulted: deletion can race startup, and a branch edit must not change the trusted teardown. */
  private async ensureDurableWorktreeHookSnapshot(
    meta: SessionMeta,
    worktree: SessionWorktreeView,
  ): Promise<WorktreeHookSnapshot | undefined> {
    const existing = this.worktreeHookSnapshot(meta, worktree);
    if (existing) return structuredClone(existing);
    if (!worktree.baseCommit || !worktree.setup ||
        worktree.setup.status === "awaiting_trust" || worktree.setup.status === "declined") return undefined;
    if (!(await this.worktreeSetupTrust.isApproved(meta.repoPath, worktree.setup.configHash))) {
      return undefined;
    }
    const loaded = await loadWorktreeSetupConfig(meta.context, meta.repoPath, worktree.baseCommit);
    if (!loaded || loaded.hash !== worktree.setup.configHash) {
      throw new Error("approved worktree configuration no longer matches its immutable identity");
    }
    const snapshot: WorktreeHookSnapshot = {
      configHash: loaded.hash,
      environment: structuredClone(loaded.config.environment),
      teardown: structuredClone(loaded.config.teardown ?? []),
    };
    this.persistWorktreeHookSnapshot(meta.sessionId, worktree, snapshot);
    return snapshot;
  }

  private protectedWorktreeEnvironment(
    meta: SessionMeta,
    worktree: SessionWorktreeView,
    environment: Record<string, string> = {},
  ): Record<string, string> {
    return resolvedWorktreeSetupEnvironment({
      version: 1,
      copyFiles: [],
      environment,
      setup: [],
  ...
  private persistWorktreeView(sessionId: string, worktree: SessionWorktreeView): void {
    const latest = this.store.readMeta(sessionId);
    if (!latest) return;
    const worktrees = this.attributedWorktrees(latest)
      .filter((item) => !sameWorktreePath(latest.context, item.path, worktree.path));
    worktrees.push(structuredClone(worktree));
    const updated = this.store.patchMeta(sessionId, { worktrees });
    if (updated) this.send({ type: "session_runtime_updated", snapshot: this.snapshot(updated) });
  }
  ...
  private persistWorktreeTeardownState(
    sessionId: string,
    worktreePath: string,
    state: WorktreeTeardownState,
  ): void {
    const latest = this.store.readMeta(sessionId);
    if (!latest) return;
    const worktrees = this.attributedWorktrees(latest).map((item) =>
      sameWorktreePath(latest.context, item.path, worktreePath)
        ? { ...item, teardown: structuredClone(state) }
        : item);
    const updated = this.store.patchMeta(sessionId, { worktrees });
    if (updated) this.send({ type: "session_runtime_updated", snapshot: this.snapshot(updated) });
  }
  ...
  private worktreeSetupApprovalInput(
    ...
    const exact = JSON.stringify({
      ...
      teardown: (config.teardown ?? []).map((step) => ({
        name: step.name,
        command: step.command,
        timeoutSeconds: step.timeoutSeconds,
        optional: step.optional,
  ...
  private async prepareWorktreeSetupAttempt(
    ...
    const priorPortBlock = worktree.portBlock;
    this.ensureWorktreePortBlock(meta, worktree);
    ...
    if (!trusted) {
      ...
      const awaiting: WorktreeSetupState = {
        ...
        environmentKeys: [...Object.keys(loaded.config.environment),
          ...
          "WOLLIPOG_WORKTREE_PATH", "WOLLIPOG_PRIMARY_CHECKOUT",
          "WOLLIPOG_PORT_BLOCK_START", "WOLLIPOG_PORT_BLOCK_END",
          "WOLLIPOG_PORT_BLOCK_SIZE"].sort(),
      ...
      if (decision === "declined") {
        ...
        // Declining repository-controlled setup suppresses only that configuration. The stable
        // runner-owned port identity is independent and must still reach the agent process.
        this.setWorktreeSetupEnvironment(meta.sessionId, worktree.path, portEnvironment);
    ...
    try {
      state = await runWorktreeSetup({
        ...
        signal,
        descendantMarker: processMarker,
    ...
    const operation = this.runWorktreeOperation(sessionId, async () => {
      ...
      try {
        ...
        if (created.created && !(error as { retainWorktree?: boolean }).retainWorktree) {
          try {
            const cleanupMeta = this.store.readMeta(sessionId) ?? meta;
            const cleanup = this.cleanupRecordForWorktree(
              cleanupMeta,
              worktree,
              "creation_rollback",
              "force",
  ...
  /** A worktree captured by an unfinished provider transition cannot yet be retired with an exact
   * process boundary. A provider already running in the target cwd is different: cleanup owns and
   * retires that process below. */
  private transitioningProviderUsesPath(sessionId: string, context: AgentContext, path: string): boolean {
    const active = this.active.get(sessionId);
    if (active?.pendingWorktreeRebind &&
        sameWorktreePath(active.context, active.pendingWorktreeRebind, path) &&
  ...
  private cleanupRecordForWorktree(
    meta: SessionMeta,
    worktree: SessionWorktreeView,
    trigger: NonNullable<WorktreeCleanupRecord["trigger"]>,
    ...
    return {
      ...
      context: meta.context,
      branch: worktree.branch,
      source: worktree.source === "created" ? "created" : "legacy",
      ...(worktree.baseRef ? { baseRef: worktree.baseRef } : {}),
      trigger,
      removalMode,
      createdAt: Date.now(),
      ...(this.worktreeHookSnapshot(meta, worktree)
        ? { hooks: structuredClone(this.worktreeHookSnapshot(meta, worktree)!) }
        : {}),
      execution: {
        ...
        args: [...meta.args],
        ...(meta.config.permissionMode ? { permissionMode: meta.config.permissionMode } : {}),
        ...(meta.executionTarget ? { executionTarget: structuredClone(meta.executionTarget) } : {}),
        ...(meta.executionHandoff ? { executionHandoff: structuredClone(meta.executionHandoff) } : {}),
        ...(meta.cloudAdapterHandoffKey ? { cloudAdapterHandoffKey: meta.cloudAdapterHandoffKey } : {}),
      },
      ...(worktree.portBlock ? { portBlock: { ...worktree.portBlock } } : {}),
      ...(worktree.teardown ? { teardown: structuredClone(worktree.teardown) } : {}),
      ...(meta.worktreeProcessMarkers?.[worktree.id]
        ? { processMarker: meta.worktreeProcessMarkers[worktree.id] }
        : {}),
      ...(checkpointOwnerHash ? { checkpointOwnerHash } : {}),
      ...(verifiedMergedHead ? { verifiedMergedHead } : {}),
    };
  }

  private async retireWorktreeProcesses(
    sessionId: string,
    context: AgentContext,
    path: string,
    ...
    if (entry && sameWorktreePath(entry.context, entry.cwd, path)) {
      this.stop(sessionId);
    ...
    const rebinding = this.worktreeRebindings.get(sessionId);
    if (rebinding && (sameWorktreePath(rebinding.entry.context, rebinding.entry.cwd, path) ||
        !!rebinding.launchingWorktreePath && sameWorktreePath(context, rebinding.launchingWorktreePath, path))) {
      this.stop(sessionId);
    ...
    if (this.liveWorktreeUsesPath(sessionId, path)) {
      throw new Error("runner-started processes still use the worktree after retirement");
    }
  }

  private persistCleanupTeardownState(record: WorktreeCleanupRecord, state: WorktreeTeardownState): void {
    record.teardown = structuredClone(state);
    this.cleanupJournal.add(record);
    this.persistWorktreeTeardownState(record.sessionId, record.worktreePath, state);
  }

  private async runCleanupTeardown(record: WorktreeCleanupRecord, meta?: SessionMeta): Promise<void> {
    const hooks = record.hooks;
    if (!hooks || hooks.teardown.length === 0 || record.teardown?.completedAt) return;
    const portBlock = record.portBlock;
    const config = {
      version: 1 as const,
      copyFiles: [],
    ...
    const hostEnvironment = resolvedWorktreeSetupEnvironment(config, {
      primaryCheckout: record.repoPath,
      worktreePath: record.worktreePath,
      branch: record.branch ?? `agent/${record.sessionId}`,
    ...
    try {
      isolation = await this.resolveCleanupIsolation(record, meta);
      environment = this.worktreeSetupRuntimeEnvironment(isolation, hostEnvironment);
      isolation = this.worktreeSetupIsolationEnvironment(isolation, environment);
    } catch (error) {
      const now = Date.now();
      this.persistCleanupTeardownState(record, {
        status: "completed_with_failures",
        configHash: hooks.configHash,
        attemptId: record.teardown?.attemptId ?? randomUUID(),
        startedAt: record.teardown?.startedAt ?? now,
        completedAt: now,
        steps: hooks.teardown.map((step) => ({
          name: step.name,
          status: "failed",
          optional: step.optional,
          startedAt: now,
          durationMs: 0,
          error: `teardown isolation unavailable: ${errText(error)}`,
          stdout: "",
          stderr: "",
        })),
      });
    ...
    const state = await runWorktreeTeardown({
      ...
      worktreePath: record.worktreePath,
      configHash: hooks.configHash,
      steps: hooks.teardown,
      environment,
      isolation,
      descendantMarker: record.processMarker,
      prior: record.teardown,
      onState: (next) => {
        this.persistCleanupTeardownState(record, next);
        next.steps.forEach((step, index) => {
          if (step.status === "running" || reported.get(index) === step.status) return;
          reported.set(index, step.status);
          if (!this.store.has(record.sessionId)) return;
          const exit = step.exitCode != null ? ` (Exit ${step.exitCode})` : step.signal ? ` (${step.signal})` : "";
          this.emitEvent(record.sessionId, {
            kind: "command_output",
            text: `[Worktree Teardown — ${step.name}]\n${step.status === "completed" ? "Completed" :
              step.status === "uncertain" ? "Uncertain (Not Replayed)" : step.optional ? "Failed (Optional)" : "Failed"}` +
              `${exit}${step.durationMs == null ? "" : ` in ${step.durationMs} ms`}${step.error ? `: ${step.error}` : ""}`,
          });
        });
      },
      onOutput: (stepIndex, text) => {
        ...
        this.emitEvent(record.sessionId, {
          kind: "command_output",
          text: `[Worktree Teardown — ${hooks.teardown[stepIndex]?.name ?? `Step ${stepIndex + 1}`}]\n${text}`,
        });
      },
    });
  ...
  private async resolveCleanupIsolation(
    record: WorktreeCleanupRecord,
    meta?: SessionMeta,
  ): Promise<SpawnIsolation | undefined> {
    if (meta) return this.resolveLaunchIsolation(meta, record.worktreePath);
    const execution = record.execution;
    // Compatibility for cleanup records written before durable execution identity existed. Such
    // records cannot name a remote target and retain the former host-only replay behavior.
    if (!execution) {
      return this.resolveIsolation(this.executionIsolation, record.context, {}, {
        driver: "acp",
    ...
    const target = execution.executionTarget;
    if (target?.adapter === "container") {
      if (!this.containerTargets || !execution.agentId) {
        throw new Error("container execution target is not configured for this cleanup");
      }
      return this.containerTargets.isolation(
        target,
        execution.agentId,
        execution.command,
        execution.args,
    ...
    if (target?.adapter === "cloud") {
      const handoffKey = execution.cloudAdapterHandoffKey;
      if (!this.cloudTargets || !execution.agentId || !handoffKey ||
          execution.executionHandoff?.targetId !== target.id) {
        throw new Error("cloud execution handoff is not available for this cleanup");
      }
    ...
    this.assertHostIsolationContextSupported(isolationMeta);
    const additionalWritableRoots = await this.requestedWorktreeIsolation(isolationMeta);
    return this.resolveIsolation(this.executionIsolation, record.context, {}, {
      driver: execution.driver,
      dataDir: this.stateDir,
      env: {},
  ...
  private async removeRecordedWorktree(record: WorktreeCleanupRecord, meta?: SessionMeta): Promise<SafeWorktreeDiscardResult> {
    ...
    const beforeRemove = async () => {
      ...
      record.processesTerminatedAt = Date.now();
      this.cleanupJournal.add(record);
      await this.runCleanupTeardown(record, meta);
    };
    if (record.removalMode === "safe") {
      return this.discardSessionWorktreeIfSafe(
        record.repoPath,
        record.sessionId,
        {
  ...
  private finishWorktreeCleanup(record: WorktreeCleanupRecord): boolean {
    try {
  ...
  private logRetainedRefState(record: RetainedWorktreeRefRecord): void {
    // Without an owner-scoped salt, legacy session and cleanup coordinates could make diagnostic
    // digests linkable. Legacy/no-owner managers keep diagnostics on the offline state-doctor
    // surface instead of emitting those identifiers.
    ...
    this.log(
      ...
      `record=${diagnostic.recordId} generation=${diagnostic.generationId} ` +
  ...
  private async discardWorktreeLocked(
    ...
    try {
      await this.ensureDurableWorktreeHookSnapshot(meta, worktree);
      meta = this.store.readMeta(sessionId) ?? meta;
    } catch (error) {
      teardownIdentityError = errText(error);
      this.log(`worktree cleanup configuration recovery failed for ${boundedSessionIdForLog(sessionId)}: ${teardownIdentityError}`);
    }
    ...
    // the lease for the worktree it is using; cleanup may transfer that exact local lease so it can
    // retire the provider, while a provider using a finished sibling remains untouched.
    ...
    try {
      // The durable record precedes process retirement/teardown/removal. Safe Git preflight still
      // runs first inside discardWorktreeIfSafe, so a dirty or unpushed tree is untouched.
      const cleanup = this.cleanupRecordForWorktree(
        meta,
        worktree,
        options.trigger ?? "explicit_discard",
        "safe",
      );
      if (teardownIdentityError) {
        const now = Date.now();
        cleanup.teardown = {
          status: "completed_with_failures",
          configHash: worktree.setup?.configHash ?? "unavailable",
          attemptId: randomUUID(),
          startedAt: now,
          completedAt: now,
          steps: [{
      ...
      this.cleanupJournal.add(cleanup);
      const result = await this.removeRecordedWorktree(cleanup, meta);
      if (!result.removed) {
            ...
            for (const path of candidatePaths) {
              ...
              if (state === "merged" || state === "closed") {
                await this.discardWorktreeLocked(candidate.sessionId, path, {
                  refreshMergedHead: false,
                  trigger: "pull_request_reconciliation",
                });
              }
            }
          });
  ...
  reconcileStore(): void {
    ...
    for (const m of this.store.listSessions()) {
      ...
      let reconciled = m;
      const migratedWorktrees = this.attributedWorktrees(reconciled).map((worktree) => ({ ...worktree }));
      let portsChanged = false;
      for (const worktree of migratedWorktrees) {
        if (worktree.source === "attached") continue;
        try {
          const previous = worktree.portBlock;
          this.ensureWorktreePortBlock(reconciled, worktree);
          portsChanged ||= !previous;
        } catch (error) {
          this.log(`worktree port allocation failed for ${boundedSessionIdForLog(m.sessionId)}: ${errText(error)}`);
        }
      }
      if (portsChanged) {
        reconciled = this.store.patchMeta(m.sessionId, { worktrees: migratedWorktrees }) ?? reconciled;
      }
      for (const worktree of migratedWorktrees) {
        if (worktree.source === "attached") continue;
        void this.ensureDurableWorktreeHookSnapshot(reconciled, worktree).catch((error) => {
          this.log(`worktree teardown identity migration failed for ${boundedSessionIdForLog(m.sessionId)}: ${errText(error)}`);
        });
      }
  ...
  private async startGeneration(
    ...
    const meta: SessionMeta = {
      ...
      pendingProviderCredentialHome: prior?.pendingProviderCredentialHome,
      providerAccountSwitchFailure: prior?.providerAccountSwitchFailure,
    ...
    const launchWorktreeCleanup = (): WorktreeCleanupRecord => {
      ...
      const latest = this.store.readMeta(spec.sessionId) ?? meta;
      if (worktreeIdentity) {
        return this.cleanupRecordForWorktree(
          latest,
          { ...worktreeIdentity, path: worktree.path, branch: worktree.branch },
          "creation_rollback",
          "force",
        );
      }
        ...
        if (gitRepo) {
          ...
          const priorActiveWorktree = activePrior?.repoPath === repoPath &&
            ...
            ? activePrior.worktrees?.find((item) =>
              (item.source !== "legacy" || !!item.baseCommit || !!item.setup || !!item.portBlock) &&
  ...
  private idleProviderCanPark(sessionId: string, entry: ActiveSession): boolean {
    ...
    return !!meta && entry.status === "idle" && !entry.running && !entry.providerInitiatedTurnActive &&
      entry.queue.length === 0 && !entry.pendingWorktreeRebind && !entry.pendingProviderAccountSwitch &&
  ...
  private async verifySelectedWorktreeBeforeLaunch(
    ...
    // Port/setup preparation may have enriched the derived legacy view before this final identity
    // proof. Do not publish that expected branch as verified after Git reports a different tree;
    // the selected path stays retained and the allocator journal keeps its stable port ownership.
    const current = this.store.readMeta(meta.sessionId);
    if (current?.worktrees?.some((item) => item.id === "legacy" &&
        sameWorktreePath(current.context, item.path, worktree.path))) {
      this.store.patchMeta(meta.sessionId, {
        worktrees: current.worktrees.filter((item) => !(item.id === "legacy" &&
          sameWorktreePath(current.context, item.path, worktree.path))),
      });
    }
  ...
  prompt(
    ...
    const dismissedAuthenticationCommands = this.store.readMeta(sessionId)?.providerAuthDismissedCommandIds;
    if (durable && dismissedAuthenticationCommands?.includes(durable.commandId)) {
      durable.failed(
  ...
  private releaseSteering(operation: SteeringOperation): void {
    ...
    if (entry) {
      ...
      if (!this.steerFences(entry).size &&
          (entry.queue.length || entry.pendingProviderAccountSwitch || entry.pendingWorktreeRebind) &&
  ...
  private async resumeAndPrompt(
    ...
    let fresh = this.store.readMeta(sessionId)!;
    const pendingAccount = this.pendingProviderAccount(fresh);
    let switchedAccount: BoundProviderAccount | undefined;
    if (pendingAccount) {
      try {
        switchedAccount = this.providerAccountForSession(fresh, pendingAccount.id);
      } catch (error) {
        this.parkProviderAccountSwitchFailure(sessionId, pendingAccount, errText(error));
        durable?.failed("provider account switch could not be applied", "INVALID_COMMAND");
        return;
      }
      const rebound = this.store.patchMeta(
        sessionId,
        this.providerAccountBindingPatch(fresh, switchedAccount),
      );
      if (!rebound) {
        durable?.failed("session disappeared while applying the provider account switch", "SESSION_NOT_FOUND");
        return;
      }
      this.store.flush(sessionId);
      fresh = rebound;
    ...
    if (!ok) {
      ...
      if (blocked?.providerAuthBlock?.delivery === "not_delivered" && durable && !syntheticRecovery) {
  ...
  private async drain(sessionId: string): Promise<void> {
    ...
    try {
      ...
      if (entry.pendingProviderAccountSwitch && this.providerAccountSwitchCanProceed(sessionId, entry)) {
        await this.rebindSelectedProviderAccount(sessionId, entry);
  ...
  private providerAccountForSession(meta: SessionMeta, providerAccountId: string): BoundProviderAccount {
    if (!this.resolveProviderAccount || !meta.agentId) {
      throw new Error("this session does not support provider accounts");
    }
    if (meta.executionTarget && meta.executionTarget.adapter !== "host") {
      throw new Error("provider accounts are available only for host sessions");
    }
    const account = this.resolveProviderAccount({
      sessionId: meta.sessionId,
      workspaceId: meta.workspaceId,
      workspacePath: meta.repoPath,
      agentId: meta.agentId,
      providerAccountId,
      command: meta.command,
      args: meta.args,
      env: {},
      useWorktree: meta.worktreePath != null,
      executionTarget: meta.executionTarget,
      driver: meta.driver,
      context: meta.context,
      config: meta.config,
    });
    if (!account) throw new Error(`provider account '${providerAccountId}' is not configured`);
    if (!meta.providerAccountProvider || account.provider !== meta.providerAccountProvider) {
      throw new Error("the selected account belongs to a different provider");
    }
    return account;
  }

  private pendingProviderAccount(meta: SessionMeta): BoundProviderAccount | undefined {
    if (!meta.pendingProviderAccountId || !meta.pendingProviderAccountLabel ||
        !meta.pendingProviderAccountProvider || !meta.pendingProviderCredentialHome) return undefined;
    return {
      id: meta.pendingProviderAccountId,
      label: meta.pendingProviderAccountLabel,
      provider: meta.pendingProviderAccountProvider,
      credentialHome: meta.pendingProviderCredentialHome,
    };
  }

  private providerAccountBindingPatch(
    meta: SessionMeta,
    ...
    return {
      ...
      ...(isProviderAuthenticationBlock(meta.pendingApproval) ? { pendingApproval: null } : {}),
    };
  }

  private parkProviderAccountSwitchFailure(
    sessionId: string,
  ...
  async switchProviderAccount(
    ...
    if (!meta.providerAccountId || !meta.providerAccountProvider || !meta.providerCredentialHome) {
      return { ok: false, error: "this session is not bound to a configured provider account" };
    }
    if (providerAccountId === meta.providerAccountId && !meta.providerAccountSwitchFailure) {
      return { ok: false, error: "the session already uses that provider account" };
    }
    ...
    if (!meta.agentSessionId && meta.seq > 0) {
      return { ok: false, error: "this provider conversation has no resumable session id" };
    }
    if (meta.agentSessionId && !canResumeSession(meta)) {
      return { ok: false, error: "this provider conversation cannot resume under another account" };
    }
    const updated = this.store.patchMeta(sessionId, {
    ...
    this.send({ type: "session_runtime_updated", snapshot: this.snapshot(updated) });
    const entry = this.active.get(sessionId);
    if (!entry) return { ok: true, scheduled: true };
    entry.pendingProviderAccountSwitch = target;
    if (entry.authenticationBlocked) entry.authenticationBlocked = false;
    if (entry.running || entry.providerInitiatedTurnActive || !this.providerAccountSwitchCanProceed(sessionId, entry)) {
  ...
  private providerAccountSwitchCanProceed(sessionId: string, entry: ActiveSession): boolean {
    const meta = this.store.readMeta(sessionId);
    const backgroundWorkBlocksSwitch =
      (!!meta?.backgroundWorkState || !!meta?.pendingBackgroundTaskIds?.length) &&
      !meta?.orphanedWork?.recoveryAttemptedAt;
    return !entry.running && !entry.providerInitiatedTurnActive &&
      this.active.get(sessionId) === entry &&
      !backgroundWorkBlocksSwitch &&
      !this.rewinding.has(sessionId) &&
      !this.forking.has(sessionId) &&
      !this.loggingOut.has(sessionId) &&
      !this.closing.has(sessionId) &&
      ...
      !entry.governanceTripped &&
      !this.queueHeld(entry) &&
      (!this.hasPendingApproval(sessionId) || isProviderAuthenticationBlock(meta?.pendingApproval)) &&
      !this.steerFences(entry).size &&
      !this.reservedPromotionPrecedesQueue(sessionId, entry);
  }

  private rebindSelectedProviderAccount(sessionId: string, entry: ActiveSession): Promise<void> {
    const current = this.providerAccountSwitches.get(sessionId);
    if (current) return current.promise;
    let promise: Promise<void>;
    promise = this.performSelectedProviderAccountSwitch(sessionId, entry).finally(() => {
      if (this.providerAccountSwitches.get(sessionId)?.promise === promise) {
        this.providerAccountSwitches.delete(sessionId);
      }
    });
    this.providerAccountSwitches.set(sessionId, { entry, promise });
    return promise;
  }
  ...
  private async performSelectedProviderAccountSwitch(
    ...
    const requested = entry.pendingProviderAccountSwitch;
    if (!requested || !this.providerAccountSwitchCanProceed(sessionId, entry)) return;
    const persisted = this.store.readMeta(sessionId);
    const pending = persisted ? this.pendingProviderAccount(persisted) : undefined;
    if (!persisted || !pending || pending.id !== requested.id) {
    ...
    let target: BoundProviderAccount;
    try {
      target = this.providerAccountForSession(persisted, pending.id);
    } catch (error) {
      entry.pendingProviderAccountSwitch = undefined;
  ...
  private queuedAsyncAnswer(sessionId: string, request: PendingApproval): boolean {
    return Boolean(request.async && this.active.get(sessionId)?.queue.some((prompt) =>
      prompt.recoveredQuestion?.pendingQuestion.async &&
      prompt.recoveredQuestion.requestId === request.requestId &&
      prompt.recoveredQuestion.recoveryId === request.recoveryId));
  }
  ...
  /** A recovered blocking answer crosses its approval barrier. An async answer does not cross a
   * separate blocking ask, but runs before ordinary queued messages once that ask is resolved. */
  private promoteQueuedRecoveredAnswer(sessionId: string, queue: QueuedPrompt[]): boolean {
    const blocking = this.hasPendingApproval(sessionId);
    const index = queue.findIndex((prompt) => blocking
      ? this.queuedPromptResolvesPendingQuestion(sessionId, prompt)
      : prompt.recoveredQuestion?.pendingQuestion.async === true);
    if (index < 0) return !blocking;
    if (index > 0) queue.unshift(queue.splice(index, 1)[0]!);
    return true;
  }
  ...
  private async runPrompt(
    ...
    if (recoveredQuestion) {
      ...
      const pending = currentApproval?.requestId === recoveredQuestion.requestId &&
          currentApproval.recoveryId === recoveredQuestion.recoveryId
        ? currentApproval
        : pendingRequests(currentApproval).find((request) =>
            request.async && request.requestId === recoveredQuestion.requestId &&
            request.recoveryId === recoveredQuestion.recoveryId) ??
          (recoveredQuestion.pendingQuestion.async ? recoveredQuestion.pendingQuestion : undefined);
    ...
    const publishUserEvent = () => recoveredQuestion
      ? this.emitEvent(sessionId, {
          ...
          ...(recoveredQuestion.pendingQuestion.async
            ? { occurrenceId: recoveredQuestion.recoveryId } : {}),
          answered: true,
          ...(recoveredQuestion.pendingQuestion.async ? { startsTurn: true } : {}),
    ...
    if (entry.worktree && (!recoveredQuestion || recoveredQuestion.pendingQuestion.async)) {
      const path = entry.worktree.path;
  ...
  async forkConversation(
    ...
    try {
      ...
      if (worktree) {
        let cleanup: WorktreeCleanupRecord;
        const cleanupMeta = this.store.readMeta(targetSessionId);
        const cleanupWorktree = cleanupMeta && this.attributedWorktrees(cleanupMeta)
          .find((item) => sameWorktreePath(cleanupMeta.context, item.path, worktree!.path));
        if (cleanupMeta && cleanupWorktree) {
          try {
            await this.ensureDurableWorktreeHookSnapshot(cleanupMeta, cleanupWorktree);
          } catch (snapshotError) {
            this.log(`failed to preserve fork teardown identity for ${targetSessionId}: ${errText(snapshotError)}`);
          }
          cleanup = this.cleanupRecordForWorktree(
            this.store.readMeta(targetSessionId) ?? cleanupMeta,
            cleanupWorktree,
            "creation_rollback",
            "force",
          );
          ...
          cleanup = {
            ...
            removalMode: "force",
            createdAt: Date.now(),
            ...(this.worktreePortAllocator.get(this.worktreePortOwner(targetSessionId, "legacy"))
              ? { portBlock: this.worktreePortAllocator.get(this.worktreePortOwner(targetSessionId, "legacy")) }
              : {}),
  ...
  async delete(sessionId: string): Promise<void> {
    ...
    try {
      ...
      if (meta) {
        const teardownIdentityErrors = new Map<string, string>();
        for (const worktree of this.attributedWorktrees(meta).filter((item) => item.source !== "attached")) {
          try {
            await this.ensureDurableWorktreeHookSnapshot(meta, worktree);
          } catch (error) {
            teardownIdentityErrors.set(worktree.id, errText(error));
          }
        }
        const cleanupMeta = this.store.readMeta(sessionId) ?? meta;
        const checkpointOwnerHash = this.checkpointOwnerHash(meta);
        ...
        for (const worktree of this.attributedWorktrees(cleanupMeta).filter((item) => item.source !== "attached")) {
          const cleanup = this.cleanupRecordForWorktree(cleanupMeta, worktree, "session_delete", "force");
          const teardownIdentityError = teardownIdentityErrors.get(worktree.id);
          if (teardownIdentityError) {
            const now = Date.now();
            cleanup.teardown = {
              status: "completed_with_failures",
          ...
          if (checkpointOwnerHash) cleanup.checkpointOwnerHash = checkpointOwnerHash;
          this.cleanupJournal.add(cleanup);
      ...
      for (const cleanup of worktreeCleanups) await this.reapWorktree(cleanup, false, meta ?? undefined);
      if (meta) {
  ...
  answerRecoveredQuestion(
    ...
    const pending: PendingApproval | undefined = meta.pendingApproval?.requestId === requestId &&
      ...
      : pendingRequests(meta.pendingApproval).find((request) =>
          ...
          request.recoveryId === recoveryId) ?? (() => {
            // The control plane accepted this durable answer while the card was actionable. A
            // later provider event may have displaced the runner snapshot before delivery.
            const ask = this.store.readEvents(sessionId).reverse().find((event) =>
              event.payload.kind === "question_request" && event.payload.async &&
              event.payload.requestId === requestId && event.payload.occurrenceId === recoveryId);
            if (!ask || ask.payload.kind !== "question_request") return undefined;
            return {
              requestId, recoveryId, occurrenceId: recoveryId, async: true,
              kind: "question" as const, title: ask.payload.questions[0]?.question ?? "The agent has a question",
              options: [], questions: ask.payload.questions,
            };
          })();
  ...
  private emitStatus(
    ...
    const [firstAsync, ...otherAsync] = terminal ? [] : pendingRequests(pending).filter((request) => request.async);
    this.store.patchMeta(sessionId, settled
      ? { status: projectedStatus, pendingApproval: firstAsync
          ? { ...firstAsync, ...(otherAsync.length ? { additionalRequests: otherAsync } : {}) } : null,
  ...
  private onDriverEvent(sessionId: string, payload: SessionEventPayload): void {
    ...
    if (payload.kind === "question_request" && payload.async) {
      for (const old of pendingRequests(this.store.readMeta(sessionId)?.pendingApproval)) {
        if (old.async && old.requestId !== payload.requestId && !this.queuedAsyncAnswer(sessionId, old)) {
    ...
    if (managedAttentionResolution && entry) {
      this.emitStatus(sessionId, pendingRequests(this.store.readMeta(sessionId)?.pendingApproval)
        .some((request) => !request.async)
  ...
  /** Persist a known-undelivered durable prompt before acknowledging it as held. The durable
   * command journal is the at-most-once fence; the runner-local copy supplies restart-safe FIFO
   * and launch configuration while the control plane continues to own the source payload. */
  private retainDurableProviderAuthenticationPrompt(
    meta: SessionMeta,
    text: string,
    images: PromptImageInput[],
    ...
    if (!existing) {
      ...
      const retainedCount = retries.length + (block.retry ? 1 : 0);
      const retainedBytes = retries.reduce(
        (total, retry) => total + queuedPromptBytes(retry.text, retry.images),
        block.retry ? queuedPromptBytes(block.retry.text, block.retry.images) : 0,
      );
      if (retainedCount >= MAX_QUEUED_PROMPTS ||
          retainedBytes + queuedPromptBytes(text, images) > MAX_QUEUED_BYTES) {
        durable.failed("prompt queue is full while provider authentication is blocked", "QUEUE_FULL");
        return false;
      }
      const retry = {
        commandId: durable.commandId,
        ordinal: reservedOrdinal ?? this.nextQueueOrdinal(meta.sessionId),
        text,
        images,
        ...(slashCommand ? { slashCommand } : {}),
        ...(config ? { config } : {}),
      ...
      this.store.patchMeta(meta.sessionId, {
        providerAuthBlock: {
          ...block,
          durableRetries: [...retries, retry].sort((left, right) => left.ordinal - right.ordinal),
        },
      });
      // This copy and its command identity must reach disk before the nonterminal receipt tells
      // the control plane that automatic recovery owns the submission.
      this.store.flush(meta.sessionId);
    } else {
      this.ensureQueueOrdinal(meta.sessionId, existing);
    }
    this.providerAuthDurables.set(durable.commandId, durable);
    const queuedGuidance = block.resolution === "approved"
      ? "Authentication was restored. This message is queued and will be submitted once its durable delivery record is recovered."
      : "Provider authentication is required. This message was not submitted and is queued for automatic delivery after authentication succeeds; no resubmission is needed.";
    durable.queued(queuedGuidance, "PROVIDER_AUTHENTICATION_REQUIRED");
    this.settleResolvedProviderAuthentication(meta.sessionId);
    return true;
  }

  /** Apply an already-recorded authentication decision once every durable journal handle needed
   * for the ordered replay has been recovered. A restart can leave the decision waiting here
   * briefly until the control-plane outbox redelivers each stable command identity. */
  private settleResolvedProviderAuthentication(sessionId: string): void {
    const meta = this.store.readMeta(sessionId);
    const block = meta?.providerAuthBlock;
    ...
    const retries = [
      ...(block.retry && meta.providerAuthRetryAttemptedRecoveryId !== block.recoveryId
        ? [{ ...block.retry, ordinal: this.ensureQueueOrdinal(sessionId, block.retry) }]
        : []),
      ...durableRetries.map((retry) => ({
        ...retry,
    ...
    // Clear the admission barrier durably before any replay can create a user event. The durable
    // command journal and command-tagged event then provide the two at-most-once boundaries.
    this.store.flush(sessionId);
    const entry = this.active.get(sessionId);
  ...
  private emitProviderAuthenticationCard(
    ...
    if (block.resolution || this.providerAuthRevalidations.has(block.credentialScopeId) ||
  ...
  private parkProviderAuthentication(
    ...
    const block: NonNullable<SessionMeta["providerAuthBlock"]> = {
      ...
      ...(prior?.durableRetries?.length ? { durableRetries: prior.durableRetries } : {}),
  ...
  private async resolveProviderAuthentication(
    ...
    if (optionId === "auth:dismiss") {
      ...
      if (retainedPrompt) {
        this.emitEvent(sessionId, {
          kind: "stderr",
          text: durableRetries.length
            ? "Authentication recovery was dismissed. Retained messages were not sent and remain available for explicit retry."
            : "Authentication recovery was dismissed. The retained prompt was not retried; submit it again if needed.",
        });
      }
      this.emitStatus(sessionId, current.status === "stopped" ? "stopped" : "idle");
  ...
  private async completeProviderAuthentication(
    ...
    for (const candidate of candidates) {
      ...
      if (retry || durableRetries.length) this.store.flush(meta.sessionId);
      const entry = this.active.get(meta.sessionId);
      if (entry) entry.authenticationBlocked = false;
  ...
  private accrueMeta(sessionId: string, payload: SessionEventPayload, trackApprovalLatency = true): void {
    if (payload.kind === "agent_message" && payload.text) {
      ...
      if (payload.kind === "question_resolved" && payload.occurrenceId &&
          !pendingRequests(before?.pendingApproval).some((request) =>
...
function providerAuthenticationGuidance(
  ...
  return [
    ...
    block?.delivery === "uncertain"
      ...
      : block?.durableRetries?.length
        ? `${block.durableRetries.length === 1 ? "One message is" : `${block.durableRetries.length} messages are`} queued for automatic delivery after authentication succeeds.`
      : block?.retry
