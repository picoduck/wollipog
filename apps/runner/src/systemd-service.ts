...
export function serviceLayout(
  ...
  if (mode === "system") {
    ...
    const dataDir = "/var/lib/wollipog";
    const configDir = "/etc/wollipog";
    return finishLayout({
...
/** Quote one systemd unit-file value (Environment=, ExecStart= arguments). */
...
export function renderControlPlaneUnit(layout: ServiceLayout, options: ControlPlaneServiceOptions): string {
  ...
  const lines = [
    ...
    `WorkingDirectory=${layout.controlPlaneDataDir}`,
    `EnvironmentFile=${layout.controlPlaneEnvFile}`,
    ...
    ...(layout.mode === "system"
      ? [
          ...
          `ReadWritePaths=${layout.controlPlaneDataDir}`,
          "PrivateTmp=yes",
        ]
      : []),
    "",
...
export function renderRunnerUnit(layout: ServiceLayout, options: { executable: string }): string {
  ...
  const lines = [
    ...
    "Documentation=https://github.com/picoduck/wollipog/blob/main/docs/headless-deployment.md",
    `After=network-online.target ${layout.units.controlPlane}`,
    `Wants=network-online.target ${layout.units.controlPlane}`,
    "StartLimitIntervalSec=300",
    "StartLimitBurst=10",
    "",
    "[Service]",
    "Type=simple",
    ...(layout.mode === "system" ? [`User=${layout.account}`, `Group=${layout.account}`] : []),
    `WorkingDirectory=${layout.runnerDataDir}`,
...
/**
 * The control-plane env file written by `wollipog service install`, so `wollipog admin` finds the
 * installed database and port without the operator exporting anything. User mode is checked
 * first, then system mode. Only a regular, non-symlink file is honoured.
 */
export function readInstalledControlPlaneEnv(
  input: { home: string; env: NodeJS.ProcessEnv; platform: NodeJS.Platform },
): InstalledControlPlaneEnv | null {
  if (input.platform !== "linux") return null;
  const candidates = [
    serviceLayout("user", { home: input.home, user: "", env: input.env }).controlPlaneEnvFile,
    serviceLayout("system", { home: input.home, user: "", env: input.env }).controlPlaneEnvFile,
  ];
