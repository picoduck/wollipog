...
function shellScript(value: unknown): string | null {
  ...
  return isAbsolute(value[0]) && (shell === "sh" || shell === "bash" || shell === "zsh")
    ? value[2] : null;
}
...
function legacyAdmission(item: ReturnType<typeof completedCommandItem>, occurrenceId: string): boolean {
  if (!item || !item.script.includes("curl ") || !item.script.includes("-X POST") ||
      !item.script.includes("$WOLLIPOG_SESSION_TOKEN_FILE") ||
      !item.script.includes("x-wollipog-agent-session") ||
      !item.script.includes(`/workflow-decisions/${occurrenceId}/consume`)) return false;
  try {
...
async function wslRollout(distro: string, threadId: string, codexHome?: string): Promise<string | null> {
  const script = [
    ...
    'matches=$(find "$root/sessions" -type f -name "*$id*.jsonl" -print 2>/dev/null | head -2)',
