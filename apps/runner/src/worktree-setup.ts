...
const RESERVED_PROCESS_ENV = new Set([
  ...
  "CONTAINERS_HELPER_BINARY_DIR", "CONTAINERS_STORAGE_CONF", "STORAGE_DRIVER", "STORAGE_OPTS",
...
const RESERVED_TOOL_ENV = /^(?:GIT|SSH)_/iu;
const RESERVED_RUNTIME_INJECTION_ENV = /^(?:PYTHONPATH|PYTHONHOME|PYTHONSTARTUP|RUBYOPT|RUBYLIB|PERL5OPT|PERL5LIB|BASH_ENV|ENV|SHELLOPTS|ZDOTDIR|NODE_PATH|JAVA_TOOL_OPTIONS|JDK_JAVA_OPTIONS|_JAVA_OPTIONS|DOTNET_STARTUP_HOOKS|COREHOST_TRACEFILE|RUSTC_WRAPPER|RUSTC_WORKSPACE_WRAPPER|MAKEFILES|MAKEFLAGS)$/iu;

function reservedSetupEnvironmentName(name: string): boolean {
  return RESERVED_ENV.test(name) || /^_CONTAINERS_/iu.test(name) || sensitiveEnvironmentName(name) ||
    ...
    RESERVED_TOOL_ENV.test(name) || RESERVED_RUNTIME_INJECTION_ENV.test(name);
}
