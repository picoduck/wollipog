...
export function registerMachineSkillRoutes(app: FastifyInstance, deps: SkillsRouteDeps): void {
  ...
  const snapshotAvailable = (runnerId: string, reply: FastifyReply) => {
    ...
    if (runner.os === "macos" && !runnerSupportsProtocol(runner.protocolVersion, "portableMachineSkillSnapshots")) {
      reply.code(409).send({ error: runnerCapabilityRequirement(
        runner.protocolVersion, "portableMachineSkillSnapshots", "macOS machine skill snapshots",
      ) });
      return false;
    }
    if (runner.os !== "linux" && runner.os !== "macos") {
      reply.code(409).send({ error: "Machine skill snapshots currently require a Linux or macOS runner." });
      return false;
    }
    return true;
  };
