...
export async function installSkillMatrixFixture(page: Page) {
  ...
  await page.route("**/api/runners/*/skills", route => route.fulfill({ json: {
    desired: [{ name: "code-review", versionDigest: "old", targets: [{ agentId: "claude", invocation: "manual" }] }],
    reported: { updatedAt: 1700000000000, deployed: [{ name: "code-review", digest: "old", links: [{ agentId: "claude", status: "linked" }, { agentId: "codex", status: "linked" }] }] }, removalReporting: "supported",
  } }));
