...
for (const width of [320, 390, 700, 1280]) {
  test(`background work remains visible through lifecycle transitions at ${width}px`, async ({ page }) => {
    ...
    const badge = header.locator(".session-header-background-work .background-work-badge, .session-header-statuses > .background-work-badge");
    for (const [state, label] of [
      ...
      if (width <= 700) {
        expect(await badge.evaluate((element) => element.getBoundingClientRect().bottom <=
          document.querySelector(".session-header-statuses")!.getBoundingClientRect().top)).toBe(true);
      }
    ...
    await expect(header.locator(".session-header-background-work")).toHaveCount(0);
  });
}
