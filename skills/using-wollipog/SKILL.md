...
To read a long range, such as a child's full final report, page forward: start with `--after 0` (or
the last seq you already read), then repeat with `--after <lastSeq> --event-epoch <eventEpoch>` from
the previous page until `hasMore` is false. `historyIncomplete` means the control plane has not yet
loaded the whole log, so later events may exist: keep the returned `lastSeq` and retry later instead
of treating the page as the end. If the call fails because the event history was replaced, restart
from `--after 0` without the old epoch. The MCP `get_session_events` tool takes the same `after`,
`limit`, and `eventEpoch`.
