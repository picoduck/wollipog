...
Once all children are finished, stop and archive only this campaign's descendants using Wollipog's supported lifecycle operations. Archive itself supports stop-before-archive. Verify `archived: true` and absence of pending/failed stop or archive state; an accepted request is not completed archival. Never archive the parent or unrelated sessions.
