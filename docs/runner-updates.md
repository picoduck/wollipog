...
Machine's npm proxy/registry configuration. A published version is not automatically known compatible
...
failed check establishes no current release status. After resolving the check failure, an
organization owner or admin can rediscover the reported installation from Connections with
**Rediscover** for a native Machine, or **Reconnect** for an SSH Machine after active sessions
finish. An unavailable integration shows its availability reason separately from release status,
since an upgrade may be the way to restore compatibility.
...
For a native Windows `.cmd`, `.bat`, extensionless, or ambiguously dotted launch such as `codex.v2`,
Agent Details lists the executable and argument array as reference data instead of a copyable command.
PowerShell routes these wrappers through `cmd.exe`,
