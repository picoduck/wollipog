...
- **WSL**: a Machine can host native and WSL agents (`runner_agents.context`). On Windows, the
  ...
  and refuses another runner owner for the same provider home. WSL failures are reported per target
  and never fall back to mutating the distribution through host path APIs. Standalone WSL runners
  report Linux and use the ordinary Linux reconciler.
