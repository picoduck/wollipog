...
shares a limit of 16 videos / 256 MiB in one Session. A separate limit of 4,096 artifacts / 1 GiB
per Session covers every author and kind, including prompt images and workflow output. Duplicate
...
An expired attachment remains named in the Session timeline but its preview reports that the
artifact is unavailable. Session and runner deletion still remove their artifact rows and queue
the same blob collection; stopping or disconnecting either does not.
